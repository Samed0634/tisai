...
export const GrevOylamasiTable: React.FC<GrevOylamasiTableProps> = ({ 
  ...
  const { visibleColumns, toggleColumn } = useColumnVisibility("oylamaColumns");
  ...
  const handleChange = (field: string, value: string | number) => {
    if (editData && field === 'GREV OYLAMASI TARİHİ') {
      setEditData({
        ...editData,
        [field]: value
      });
            ...
            {data.length === 0 ? (
              ...
              data.map((item) => (
                <TableRow key={item.ID} className="hover:bg-muted/50">
                  ...
                  {visibleColumnDefinitions.map(column => (
                    <TableCell key={`${item.ID}-${column.id}`} className={cn(
                      column.id === 'GREV OYLAMASI TARİHİ' && "bg-yellow-50"
                    )}>
                      {editingId === item.ID && editData && column.id === 'GREV OYLAMASI TARİHİ' ? (
                        <Input 
                          type="date"
